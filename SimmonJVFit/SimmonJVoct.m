...
plot(V,J)
xlabel('Voltage (V)')
ylabel('Current density (A/m^{2})','interpreter','tex')
title('J vs V plot')
f=gcf;
...
p0=[3.0, 0.9, 0.8]
%%%%%%%%%%%%%%%%%fitting%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
options = optimset("MaxIter",10000,"Algorithm","lm_svd_feasible"); 
lb = [];
ub = [];
phi=[];
meff=[];
...
[coeff,resnorm,residual,exitflag,output, lambda, jacobian] = lsqcurvefit(@(varargin)f(varargin{:}),p0,V(:,i),J(:,i),lb,ub,options);
p0=coeff
Coeff(end+1,:)=p0
phi(end+1)=coeff(1);
meff(end+1)=coeff(2);
